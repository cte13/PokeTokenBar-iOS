...
final class UsageStore {
    ...
    var limitsReady: Bool {
        limits != nil || !additionalLimits.isEmpty || codexLimits != nil || opencodeGoLimits != nil
            || antigravityLimits != nil || cursorLimits != nil
    }
    ...
    private func runAdditionalClaudeLimitsRefresh(allowKeychainPrompt: Bool) async {
        ...
        for root in roots {
            ...
            // Same poll cadence as the default account (LimitsPollCadence): the usage-scan interval must
            // not set how often each folder's token hits the limits endpoint. A manual refresh bypasses it.
            let attemptKey = "claude_code:" + root.path
            if !allowKeychainPrompt,
               !LimitsPollCadence.shouldFetch(lastAttemptAt: lastRemoteLimitsAttempt[attemptKey],
                                              minimumInterval: remoteLimitsPollInterval) {
                if let previous { keep(previous) }
                continue
            }
            lastRemoteLimitsAttempt[attemptKey] = Date()
            do {
    ...
    private func refreshCursorLimits() async {
        guard LimitsPollCadence.shouldFetch(lastAttemptAt: lastRemoteLimitsAttempt["cursor"],
                                            minimumInterval: remoteLimitsPollInterval) else { return }
        lastRemoteLimitsAttempt["cursor"] = Date()
        do {
    ...
    private func updateAuthExpired(from error: any Error) {
            ...
            // **401 만** 세션 만료다(#32). 403 은 "인증은 됐고 이 메서드를 쓸 권한이 없다"는 뜻이라
            // 재로그인으로 풀리지 않는다 — 실측(2026-08-31): antigravity CLI 자격증명이
            // retrieveUserQuotaSummary 에서 403 을 받자 "세션 만료" 안내가 영구 표시됐고 재시도
            // 버튼까지 가렸다. 업스트림은 아직 401||403 을 묶어 두지만 여기서는 분리를 유지한다.
