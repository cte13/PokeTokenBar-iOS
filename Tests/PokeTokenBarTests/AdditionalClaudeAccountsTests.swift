...
private final class CountingCursorLimits: CursorLimitsProviding, @unchecked Sendable {
...
final class AdditionalClaudeAccountsStoreTests: XCTestCase {
    ...
    private func makeStore(primary: ScriptedLimits,
                           ...
                           defaultIdentity: AccountIdentity? = nil,
                           pollInterval: TimeInterval = 0,
                           cursor: any CursorLimitsProviding = CountingCursorLimits()) -> UsageStore {
        ...
        return UsageStore(
            ...
            autoRefresh: false,
            remoteLimitsPollInterval: pollInterval,
    ...
    /// The usage-scan interval must not set how often a folder's token hits the limits endpoint
    /// (LimitsPollCadence): the default account was throttled while other folders polled every scan.
    func testAutomaticPollsRespectTheCadenceForEveryFolder() async {
        let primary = ScriptedLimits([.success(limitStatus(fiveHour: 10, email: "main@example.com"))])
        let folder = ScriptedLimits([.success(limitStatus(fiveHour: 20, email: "work@example.com"))])
        let store = makeStore(primary: primary, folders: [work: folder],
                              pollInterval: LimitsPollCadence.minimumInterval)

        await store.refresh(scheduleEmptyRetry: false)
        await store.refresh(scheduleEmptyRetry: false)
        XCTAssertEqual(folder.promptFlags, [false], "the second automatic poll is inside the cadence window")
        XCTAssertEqual(store.additionalLimits.map(\.status.fiveHour?.utilization), [20], "a skipped folder keeps its values")

        await store.refreshLimitTokenFromKeychain()
        XCTAssertEqual(folder.promptFlags, [false, true], "a manual refresh bypasses the cadence")
    }

    func testAutomaticCursorPollsRespectTheCadence() async {
        let cursor = CountingCursorLimits()
        let store = makeStore(primary: ScriptedLimits([.success(limitStatus(fiveHour: 10, email: "main@example.com"))]),
                              folders: [:], pollInterval: LimitsPollCadence.minimumInterval, cursor: cursor)

        await store.refresh(scheduleEmptyRetry: false)
        await store.refresh(scheduleEmptyRetry: false)
        XCTAssertEqual(cursor.calls, 1, "the second automatic poll is inside the cadence window")
    }
