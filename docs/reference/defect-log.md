...
- **토큰 파일 파서는 외부 도구의 실제 직렬화 구조체(중첩 객체)를 반영해야 한다.** `jetski-standalone-oauth-token`
  ...
  해결: `readTokenFile`에서 `parseCredential`을 재활용하여 중첩 객체(`access_token`),
  ...
  함께 전송한다(이 포크는 secret 을 저장소에 두지 않고 설치된 바이너리에서 런타임에 읽는다 — 아래 §vendor client_secret). 또한 만료 직전(near-expiry) 계정 전환 시 이전 계정 캐시로 잘못 폴백하지 않도록 동일 출처
...
- **팝오버 세로 `ScrollView` 는 콘텐츠 안쪽에 스크롤러 레인을 비운다 — `.reservesScrollerLane()`.**
  #59 가 홈 탭을 `ScrollView` + 고정 520pt 로 감싸자, 오른쪽 끝에 붙은 수치(오늘 비용·Peak·추이 막대
  ...
  **왜 못 걸렀나:** #59 는 "팝오버가 화면 밖으로 넘친다"만 검증했고, 스크롤러와 콘텐츠의 겹침은 어떤
  테스트도 보지 않았다. 오버레이 스크롤러는 스크롤할 때만 보여 정지 스크린샷에도 안 잡힌다.
  **수정:** 스크롤바를 숨기지 않는다(#62 의 `.scrollIndicators(.never)` 는 스크롤 위치 단서를 없애고,
